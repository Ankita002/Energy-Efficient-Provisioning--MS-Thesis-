...
public class PowerVmAllocationPolicyDoubleThresholdAbstract extends VmAllocationPolicySimple {
	 ...
	 */
	public PowerVmAllocationPolicyDoubleThresholdAbstract(List<? extends PowerHost> list, double utilizationThresholdLower, double utilizationThresholdUpper) {
		super(list);
	...
	public PowerHost findHostForVm(Vm vm) {
		...
		for (PowerHost host : this.<PowerHost>getHostList()) {
			if (host.isSuitableForVm(vm)) {
				...
				if ((!vm.isRecentlyCreated() && maxUtilization > getUtilizationThresholdUpper()) || (vm.isRecentlyCreated() && maxUtilization > 1.0)) 
				{
					continue;
				}
				try {
